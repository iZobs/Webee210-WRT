...
#include <plat/regs-sdaout.h>

#include "tv_out_s5pv210.h"

#ifdef S5P_SDAOUT_DEBUG
#define SDPRINTK(fmt, args...)	\
	pr_debug("\t\t[SDOUT] %s: " fmt, __func__ , ## args)
#else
#define SDPRINTK(fmt, args...)
#endif

static struct resource	*g_sdout_mem;
static void __iomem	*g_sdout_base;

/*
* initialization  - iniization functions are only called under stopping SDOUT
*/
enum s5p_tv_sd_err s5p_sdout_init_video_scale_cfg(
	...
	enum s5p_sd_vsync_ratio composite_ratio)
{
	...
	case S5P_TV_SD_LEVEL_75IRE:
		...
		break;
	default:
		pr_err("invalid component_level parameter(%d)\n",
			component_level);
	...
	case SDOUT_VTOS_RATIO_7_3:
		...
		break;
	default:
		pr_err(" invalid composite_level parameter(%d)\n",
			composite_level);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	default:
		pr_err("invalid composite_ratio parameter(%d)\n",
			composite_ratio);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case SDOUT_VTOS_RATIO_7_3:
		...
		break;
	default:
		pr_err("invalid component_ratio parameter(%d)\n",
			component_ratio);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}

	writel(temp_reg, g_sdout_base + S5P_SDO_SCALE);

	SDPRINTK("component_level(%d), component_ratio(%d), \
		composite_level(%d), composite_ratio(%d) \
		S5P_SDO_SCALE(0x%08x)\n",
		component_level, component_ratio,
		composite_level, composite_ratio,
		readl(g_sdout_base + S5P_SDO_SCALE));

	return SDOUT_NO_ERROR;
}

enum s5p_tv_sd_err s5p_sdout_init_sync_signal_pin(
	enum s5p_sd_sync_sig_pin pin)
{
	switch (pin) {
	case SDOUT_SYNC_SIG_NO:
		writel(SDO_COMPONENT_SYNC_ABSENT, g_sdout_base + S5P_SDO_SYNC);
		break;
	case SDOUT_SYNC_SIG_YG:
		writel(SDO_COMPONENT_SYNC_YG, g_sdout_base + S5P_SDO_SYNC);
		break;
	case SDOUT_SYNC_SIG_ALL:
		writel(SDO_COMPONENT_SYNC_ALL, g_sdout_base + S5P_SDO_SYNC);
		break;
	default:
		pr_err("invalid pin parameter(%d)\n", pin);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}

	SDPRINTK("pin(%d), S5P_SDO_SYNC(0x%08x)\n",
		pin, readl(g_sdout_base + S5P_SDO_SYNC));

	return SDOUT_NO_ERROR;
}

enum s5p_tv_sd_err s5p_sdout_init_vbi(bool wss_cvbs,
			...
			enum s5p_sd_closed_caption_type caption_y_ppr)
{
	...
	case SDOUT_INS_OTHERS:
		...
		break;
	default:
		pr_err(" invalid caption_cvbs parameter(%d)\n",
			caption_cvbs);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case SDOUT_INS_OTHERS:
		...
		break;
	default:
		pr_err("invalid caption_y_sideo parameter(%d)\n",
			caption_y_sideo);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case SDOUT_INS_OTHERS:
		...
		break;
	default:
		pr_err(" invalid caption_rgb parameter(%d)\n",
			caption_rgb);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case SDOUT_INS_OTHERS:
		...
		break;
	default:
		pr_err("invalid caption_y_ppr parameter(%d)\n",
			caption_y_ppr);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}

	writel(temp_reg, g_sdout_base + S5P_SDO_VBI);

	SDPRINTK("wss_cvbs(%d), caption_cvbs(%d), \
		wss_y_sideo(%d), caption_y_sideo(%d), \
		cgmsa_rgb(%d), wss_rgb(%d), caption_rgb(%d), \
		cgmsa_y_ppr(%d), wss_y_ppr(%d), caption_y_ppr(%d), \
		S5P_SDO_VBI(0x%08x)\n",
		wss_cvbs, caption_cvbs,
		wss_y_sideo, caption_y_sideo,
		cgmsa_rgb, wss_rgb, caption_rgb,
		cgmsa_y_ppr, wss_y_ppr, caption_y_ppr,
		readl(g_sdout_base + S5P_SDO_VBI));

	return SDOUT_NO_ERROR;
}

enum s5p_tv_sd_err s5p_sdout_init_offset_gain(
	...
	case SDOUT_CHANNEL_0:
		writel(SDO_SCALE_CONV_OFFSET(offset) |
			SDO_SCALE_CONV_GAIN(gain),
		       g_sdout_base + S5P_SDO_SCALE_CH0);

		SDPRINTK("S5P_SDO_SCALE_CH0(0x%08x)\n",
			readl(g_sdout_base + S5P_SDO_SCALE_CH0));
		break;
	case SDOUT_CHANNEL_1:
		writel(SDO_SCALE_CONV_OFFSET(offset) |
			SDO_SCALE_CONV_GAIN(gain),
		       g_sdout_base + S5P_SDO_SCALE_CH1);

		SDPRINTK("S5P_SDO_SCALE_CH1(0x%08x)\n",
			readl(g_sdout_base + S5P_SDO_SCALE_CH1));
		break;
	case SDOUT_CHANNEL_2:
		writel(SDO_SCALE_CONV_OFFSET(offset) |
			SDO_SCALE_CONV_GAIN(gain),
		       g_sdout_base + S5P_SDO_SCALE_CH2);

		SDPRINTK("S5P_SDO_SCALE_CH2(0x%08x)\n",
			readl(g_sdout_base + S5P_SDO_SCALE_CH2));
		break;
	default:
		pr_err(" invalid channel parameter(%d)\n", channel);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}

	SDPRINTK("channel(%d), offset(%d), gain(%d)\n",
		channel, offset, gain);
			...
			u32 offset_video_end)
{
	writel(SDO_DELAY_YTOC(delay_y) |
		...
		g_sdout_base + S5P_SDO_YCDELAY);

	SDPRINTK("delay_y(%d), offset_video_start(%d), \
		offset_video_end(%d), \
		S5P_SDO_YCDELAY(0x%08x)\n",
		delay_y, offset_video_start,
		offset_video_end,
		readl(g_sdout_base + S5P_SDO_YCDELAY));
}

void s5p_sdout_init_schlock(bool color_sucarrier_pha_adj)
{
	if (color_sucarrier_pha_adj)
		writel(SDO_COLOR_SC_PHASE_ADJ, g_sdout_base + S5P_SDO_SCHLOCK);
	else
		writel(SDO_COLOR_SC_PHASE_NOADJ, g_sdout_base + S5P_SDO_SCHLOCK);

	SDPRINTK("color_sucarrier_pha_adj(%d), \
		S5P_SDO_SCHLOCK(0x%08x)\n",
		color_sucarrier_pha_adj,
		readl(g_sdout_base + S5P_SDO_SCHLOCK));
}

enum s5p_tv_sd_err s5p_sdout_init_dac_power_onoff(
	enum s5p_sd_channel_sel channel, bool dac_on)
{
	...
	if (dac_on)
		writel(readl(g_sdout_base + S5P_SDO_DAC) | temp_on_off,
			g_sdout_base + S5P_SDO_DAC);
	else
		writel(readl(g_sdout_base + S5P_SDO_DAC) & ~temp_on_off,
			g_sdout_base + S5P_SDO_DAC);

	SDPRINTK("channel(%d), dac_on(%d), \
		S5P_SDO_DAC(0x%08x)\n",
		channel, dac_on,
		readl(g_sdout_base + S5P_SDO_DAC));
					...
					bool y_cvbs_color_compensation)
{
	...
	writel(temp_reg, g_sdout_base + S5P_SDO_CCCON);

	SDPRINTK("bright_hue_saturation_adj(%d), \
		y_ppr_color_compensation(%d), rgcolor_compensation(%d), \
		y_c_color_compensation(%d), y_cvbs_color_compensation(%d), \
		S5P_SDO_CCCON(0x%08x)\n",
		bright_hue_saturation_adj,
		y_ppr_color_compensation, rgcolor_compensation,
		y_c_color_compensation, y_cvbs_color_compensation,
		readl(g_sdout_base + S5P_SDO_CCCON));
}
					...
					u32 offset_cr_hue_saturation)
{
	writel(SDO_BRIGHTNESS_GAIN(gain_brightness) |
		...
		g_sdout_base + S5P_SDO_YSCALE);

	writel(SDO_HS_CB_GAIN0(gain0_cb_hue_saturation) |
		SDO_HS_CB_GAIN1(gain1_cb_hue_saturation),
		g_sdout_base + S5P_SDO_CBSCALE);

	writel(SDO_HS_CR_GAIN0(gain0_cr_hue_saturation) |
		SDO_HS_CR_GAIN1(gain1_cr_hue_saturation),
		g_sdout_base + S5P_SDO_CRSCALE);

	writel(SDO_HS_CR_OFFSET(offset_cr_hue_saturation) |
		SDO_HS_CB_OFFSET(offset_cb_hue_saturation),
		g_sdout_base + S5P_SDO_CB_CR_OFFSET);

	SDPRINTK("gain_brightness(%d), offset_brightness(%d), \
		gain0_cb_hue_saturation(%d), gain1_cb_hue_saturation(%d), \
		gain0_cr_hue_saturation(%d), gain1_cr_hue_saturation(%d), \
		offset_cb_hue_saturation(%d), offset_cr_hue_saturation(%d)\n",
		gain_brightness, offset_brightness,
		gain0_cb_hue_saturation, gain1_cb_hue_saturation,
		gain0_cr_hue_saturation, gain1_cr_hue_saturation,
		offset_cb_hue_saturation, offset_cr_hue_saturation);

	SDPRINTK("S5P_SDO_YSCALE(0x%08x), S5P_SDO_CBSCALE(0x%08x), \
		S5P_SDO_CRSCALE(0x%08x), S5P_SDO_CB_CR_OFFSET(0x%08x)\n",
		readl(g_sdout_base + S5P_SDO_YSCALE),
		readl(g_sdout_base + S5P_SDO_CBSCALE),
		readl(g_sdout_base + S5P_SDO_CRSCALE),
		readl(g_sdout_base + S5P_SDO_CB_CR_OFFSET));
}
						   ...
						   u32 min_rgbcube)
{
	writel(SDO_MAX_RGB_CUBE(max_rgbcube) | SDO_MIN_RGB_CUBE(min_rgbcube),
	       g_sdout_base + S5P_SDO_RGB_CC);

	SDPRINTK("max_rgbcube(0x%08x), min_rgbcube(0x%08x) \
		S5P_SDO_RGB_CC(0x%08x)\n",
		max_rgbcube, min_rgbcube,
		readl(g_sdout_base + S5P_SDO_RGB_CC));
}
					...
					u32 radius)
{
	writel(SDO_Y_LOWER_MID_CVBS_CORN(y_lower_mid) |
		SDO_Y_BOTTOM_CVBS_CORN(y_bottom),
	       g_sdout_base + S5P_SDO_CVBS_CC_Y1);
	writel(SDO_Y_TOP_CVBS_CORN(y_top) |
		SDO_Y_UPPER_MID_CVBS_CORN(y_upper_mid),
	       g_sdout_base + S5P_SDO_CVBS_CC_Y2);
	writel(SDO_RADIUS_CVBS_CORN(radius), g_sdout_base + S5P_SDO_CVBS_CC_C);

	SDPRINTK("%d, %d, %d, %d, %d\n", y_lower_mid, y_bottom, y_top,
		y_upper_mid, radius);

	SDPRINTK("0x%08x, 0x%08x, 0x%08x)\n",
		readl(g_sdout_base + S5P_SDO_CVBS_CC_Y1),
		readl(g_sdout_base + S5P_SDO_CVBS_CC_Y2),
		readl(g_sdout_base + S5P_SDO_CVBS_CC_C));
}
					...
					u32 y_c_cylinder)
{
	writel(SDO_Y_TOP_YC_CYLINDER(y_top) |
		SDO_Y_BOTOM_YC_CYLINDER(y_bottom),
	       g_sdout_base + S5P_SDO_YC_CC_Y);
	writel(SDO_RADIUS_YC_CYLINDER(y_c_cylinder),
		g_sdout_base + S5P_SDO_YC_CC_C);

	SDPRINTK(" %d, %d, %d)\n", y_top, y_bottom, y_c_cylinder);

	SDPRINTK("0x%08x, 0x%08x)\n", readl(g_sdout_base + S5P_SDO_YC_CC_Y),
		readl(g_sdout_base + S5P_SDO_YC_CC_C));
}
				...
				u32 front_625)
{
	writel(SDO_COMPONENT_525_BP(back_525) |
		SDO_COMPONENT_525_FP(front_525),
	       g_sdout_base + S5P_SDO_CSC_525_PORCH);
	writel(SDO_COMPONENT_625_BP(back_625) |
		SDO_COMPONENT_625_FP(front_625),
	       g_sdout_base + S5P_SDO_CSC_625_PORCH);

	SDPRINTK(" %d, %d, %d, %d)\n", back_525,
		front_525, back_625, front_625);

	SDPRINTK(" 0x%08x, 0x%08x)\n",
		readl(g_sdout_base + S5P_SDO_CSC_525_PORCH),
		readl(g_sdout_base + S5P_SDO_CSC_625_PORCH));
}

enum s5p_tv_sd_err s5p_sdout_init_vesa_rgb_sync(
	...
	enum s5p_tv_active_polarity h_sync_active)
{
	...
	case SDOUT_VESA_RGB_SYNC_SEPARATE:
		...
		break;
	default:
		pr_err(" invalid sync_type parameter(%d)\n", sync_type);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case TVOUT_POL_ACTIVE_HIGH:
		...
		break;
	default:
		pr_err(" invalid v_sync_active parameter(%d)\n",
			v_sync_active);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case TVOUT_POL_ACTIVE_HIGH:
		...
		break;
	default:
		pr_err(" invalid h_sync_active parameter(%d)\n",
			h_sync_active);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}

	writel(temp_reg, g_sdout_base + S5P_SDO_RGBSYNC);

	SDPRINTK("%d, %d, %d\n", sync_type, v_sync_active, h_sync_active);
	SDPRINTK("0x%08x\n", readl(g_sdout_base + S5P_SDO_RGBSYNC));
					      ...
					      u32 *coeff2)
{
	...
	if (coeff != NULL) {
		temp_reg = (u32 *)readl(g_sdout_base + S5P_SDO_OSFC00_0);
		memcpy((void *)temp_reg, (const void *)coeff, size*4);
	}

	if (coeff1 != NULL) {
		temp_reg = (u32 *)readl(g_sdout_base + S5P_SDO_OSFC00_1);
		memcpy((void *)temp_reg, (const void *)coeff1, size*4);
	}

	if (coeff2 != NULL) {
		temp_reg = (u32 *)readl(g_sdout_base + S5P_SDO_OSFC00_2);
...
enum s5p_tv_sd_err s5p_sdout_init_ch_xtalk_cancel_coef(
	enum s5p_sd_channel_sel channel,
	u32 coeff2, u32 coeff1)
{
	...
	case SDOUT_CHANNEL_0:
		writel(SDO_XTALK_COEF02(coeff2) | SDO_XTALK_COEF01(coeff1),
			g_sdout_base + S5P_SDO_XTALK0);
		SDPRINTK(" 0x%08x)\n", readl(g_sdout_base + S5P_SDO_XTALK0));
		break;
	case SDOUT_CHANNEL_1:
		writel(SDO_XTALK_COEF02(coeff2) | SDO_XTALK_COEF01(coeff1),
			g_sdout_base + S5P_SDO_XTALK1);
		SDPRINTK(" 0x%08x)\n", readl(g_sdout_base + S5P_SDO_XTALK1));
		break;
	case SDOUT_CHANNEL_2:
		writel(SDO_XTALK_COEF02(coeff2) | SDO_XTALK_COEF01(coeff1),
			g_sdout_base + S5P_SDO_XTALK2);
		SDPRINTK("0x%08x)\n", readl(g_sdout_base + S5P_SDO_XTALK2));
		break;
	default:
		pr_err(" invalid channel parameter(%d)\n", channel);
		return S5P_TV_SD_ERR_INVALID_PARAM;
...
void s5p_sdout_init_closed_caption(u32 display_cc, u32 non_display_cc)
{
	writel(SDO_DISPLAY_CC_CAPTION(display_cc) |
		...
		g_sdout_base + S5P_SDO_ARMCC);

	SDPRINTK("%d, %d\n", display_cc, non_display_cc);
	SDPRINTK("0x%x\n", readl(g_sdout_base + S5P_SDO_ARMCC));
}
...
enum s5p_tv_sd_err s5p_sdout_init_wss525_data(
	...
	enum s5p_sd_525_aspect_ratio display_ratio)
{
	...
	case SDO_525_NOCOPY_PERMIT:
		...
		break;
	default:
		pr_err(" invalid copy_permit parameter(%d)\n", copy_permit);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case SDO_525_MV_PSP_ON_4LINE_BURST:
		...
		break;
	default:
		pr_err(" invalid mv_psp parameter(%d)\n", mv_psp);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case SDO_525_DEFAULT:
		...
		break;
	default:
		pr_err(" invalid copy_info parameter(%d)\n", copy_info);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case SDO_525_NOCOPY_PERMIT:
		...
		break;
	default:
		pr_err(" invalid display_ratio parameter(%d)\n",
			display_ratio);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	writel(temp_reg |
		SDO_CRC_WSS525(s5p_sdout_init_wss_cgms_crc(temp_reg)),
		g_sdout_base + S5P_SDO_WSS525);

	SDPRINTK("%d, %d, %d, %d\n", copy_permit, mv_psp, copy_info,
		display_ratio);

	SDPRINTK("0x%08x)\n", readl(g_sdout_base + S5P_SDO_WSS525));

	return SDOUT_NO_ERROR;
}

enum s5p_tv_sd_err s5p_sdout_init_wss625_data(bool surround_sound,
				 bool copyright,
				 ...
				 enum s5p_sd_625_aspect_ratio display_ratio)
{
	...
	case SDO_625_OUTACT_OPEN_SUBTITLES:
		...
		break;
	default:
		pr_err(" invalid open_subtitles parameter(%d)\n",
			open_subtitles);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case SDO_625_FILM:
		...
		break;
	default:
		pr_err("invalid camera_film parameter(%d)\n",
			camera_film);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case SDO_625_MOTION_ADAPTIVE_COLORPLUS:
		...
		break;
	default:
		pr_err("invalid color_encoding parameter(%d)\n",
			color_encoding);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	writel(temp_reg, g_sdout_base + S5P_SDO_WSS625);
	...
	SDPRINTK("0x%08x\n", readl(g_sdout_base + S5P_SDO_WSS625));

	return SDOUT_NO_ERROR;
}

enum s5p_tv_sd_err s5p_sdout_init_cgmsa525_data(
	enum s5p_sd_525_copy_permit copy_permit,
	enum s5p_sd_525_mv_psp mv_psp,
	enum s5p_sd_525_copy_info copy_info,
	bool analog_on,
	enum s5p_sd_525_aspect_ratio display_ratio)
{
	...
	writel(temp_reg | SDO_CRC_CGMS525(
		...
		g_sdout_base + S5P_SDO_CGMS525);

	SDPRINTK("%d, %d, %d, %d)\n", copy_permit, mv_psp, copy_info,
		display_ratio);

	SDPRINTK(" 0x%08x)\n", readl(g_sdout_base + S5P_SDO_CGMS525));

	return SDOUT_NO_ERROR;
}

enum s5p_tv_sd_err s5p_sdout_init_cgmsa625_data(bool surround_sound,
				bool copyright,
				bool copy_protection,
				bool text_subtitles,
				enum s5p_sd_625_subtitles open_subtitles,
				...
				enum s5p_sd_625_aspect_ratio display_ratio)
{
	...
	writel(temp_reg, g_sdout_base + S5P_SDO_CGMS625);
	...
	SDPRINTK("%d, %d, %d, %d, %d, %d, %d, %d, %d)\n", surround_sound,
		...
		display_ratio);
	SDPRINTK("0x%08x\n", readl(g_sdout_base + S5P_SDO_CGMS625));

	return SDOUT_NO_ERROR;
}

static enum s5p_tv_sd_err s5p_sdout_init_antialias_filter_coeff_default(
	enum s5p_sd_level composite_level,
	enum s5p_sd_vsync_ratio composite_ratio,
	enum s5p_tv_o_mode out_mode)
{
	...
	case S5P_TV_SD_LEVEL_0IRE:
		...
		case SDOUT_VTOS_RATIO_10_4:
			...
			case TVOUT_OUTPUT_SVIDEO:
				writel(0x00000000 , g_sdout_base + S5P_SDO_Y3);
				writel(0x00000000 , g_sdout_base + S5P_SDO_Y4);
				writel(0x00000000 , g_sdout_base + S5P_SDO_Y5);
				writel(0x00000000 , g_sdout_base + S5P_SDO_Y6);
				writel(0x00000000 , g_sdout_base + S5P_SDO_Y7);
				writel(0x00000000 , g_sdout_base + S5P_SDO_Y8);
				writel(0x00000000 , g_sdout_base + S5P_SDO_Y9);
				writel(0x00000000 , g_sdout_base + S5P_SDO_Y10);
				writel(0x0000029a , g_sdout_base + S5P_SDO_Y11);
				writel(0x00000000 , g_sdout_base + S5P_SDO_CB0);
				writel(0x00000000 , g_sdout_base + S5P_SDO_CB1);
				writel(0x00000000 , g_sdout_base + S5P_SDO_CB2);
				writel(0x00000000 , g_sdout_base + S5P_SDO_CB3);
				writel(0x00000000 , g_sdout_base + S5P_SDO_CB4);
				writel(0x00000001 , g_sdout_base + S5P_SDO_CB5);
				writel(0x00000007 , g_sdout_base + S5P_SDO_CB6);
				writel(0x00000015 , g_sdout_base + S5P_SDO_CB7);
				writel(0x0000002b , g_sdout_base + S5P_SDO_CB8);
				writel(0x00000045 , g_sdout_base + S5P_SDO_CB9);
				writel(0x00000059 , g_sdout_base + S5P_SDO_CB10);
				writel(0x00000061 , g_sdout_base + S5P_SDO_CB11);
				writel(0x00000000 , g_sdout_base + S5P_SDO_CR1);
				writel(0x00000000 , g_sdout_base + S5P_SDO_CR2);
				writel(0x00000000 , g_sdout_base + S5P_SDO_CR3);
				writel(0x00000000 , g_sdout_base + S5P_SDO_CR4);
				writel(0x00000002 , g_sdout_base + S5P_SDO_CR5);
				writel(0x0000000a , g_sdout_base + S5P_SDO_CR6);
				writel(0x0000001e , g_sdout_base + S5P_SDO_CR7);
				writel(0x0000003d , g_sdout_base + S5P_SDO_CR8);
				writel(0x00000061 , g_sdout_base + S5P_SDO_CR9);
				writel(0x0000007a , g_sdout_base + S5P_SDO_CR10);
				writel(0x0000008f , g_sdout_base + S5P_SDO_CR11);
				break;
			...
			case TVOUT_OUTPUT_COMPONENT_RGB_PROGRESSIVE:
				writel(0x00000000, g_sdout_base + S5P_SDO_Y0);
				writel(0x00000000, g_sdout_base + S5P_SDO_Y1);
				writel(0x00000000, g_sdout_base + S5P_SDO_Y2);
				writel(0x00000000, g_sdout_base + S5P_SDO_Y3);
				writel(0x00000000, g_sdout_base + S5P_SDO_Y4);
				writel(0x00000000, g_sdout_base + S5P_SDO_Y5);
				writel(0x00000000, g_sdout_base + S5P_SDO_Y6);
				...
				break;
			default:
				pr_err("invalid out_mode parameter(%d)\n",
					out_mode);
				return S5P_TV_SD_ERR_INVALID_PARAM;
				break;
			}
		...
		case SDOUT_VTOS_RATIO_7_3:
			...
			writel(0x00000000, g_sdout_base + S5P_SDO_Y9);
			writel(0x00000000, g_sdout_base + S5P_SDO_Y10);
			writel(0x00000281, g_sdout_base + S5P_SDO_Y11);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB0);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB1);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB2);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB3);
			...
			writel(0x00000007, g_sdout_base + S5P_SDO_CB6);
			writel(0x00000015, g_sdout_base + S5P_SDO_CB7);
			writel(0x0000002a, g_sdout_base + S5P_SDO_CB8);
			writel(0x00000044, g_sdout_base + S5P_SDO_CB9);
			writel(0x00000057, g_sdout_base + S5P_SDO_CB10);
			writel(0x0000005f, g_sdout_base + S5P_SDO_CB11);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR1);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR2);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR3);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR4);
			writel(0x00000002, g_sdout_base + S5P_SDO_CR5);
			writel(0x0000000a, g_sdout_base + S5P_SDO_CR6);
			writel(0x0000001d, g_sdout_base + S5P_SDO_CR7);
			writel(0x0000003c, g_sdout_base + S5P_SDO_CR8);
			writel(0x0000005f, g_sdout_base + S5P_SDO_CR9);
			writel(0x0000007b, g_sdout_base + S5P_SDO_CR10);
			writel(0x00000086, g_sdout_base + S5P_SDO_CR11);
			break;
		default:
			pr_err("invalid composite_ratio parameter(%d)\n",
				composite_ratio);
	...
	case S5P_TV_SD_LEVEL_75IRE:
		...
		case SDOUT_VTOS_RATIO_10_4:
			...
			writel(0x00000000, g_sdout_base + S5P_SDO_Y9);
			writel(0x00000000, g_sdout_base + S5P_SDO_Y10);
			writel(0x0000025d, g_sdout_base + S5P_SDO_Y11);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB0);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB1);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB2);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB3);
			...
			writel(0x00000001, g_sdout_base + S5P_SDO_CB5);
			writel(0x00000007, g_sdout_base + S5P_SDO_CB6);
			writel(0x00000014, g_sdout_base + S5P_SDO_CB7);
			writel(0x00000028, g_sdout_base + S5P_SDO_CB8);
			writel(0x0000003f, g_sdout_base + S5P_SDO_CB9);
			writel(0x00000052, g_sdout_base + S5P_SDO_CB10);
			writel(0x0000005a, g_sdout_base + S5P_SDO_CB11);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR1);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR2);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR3);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR4);
			writel(0x00000001, g_sdout_base + S5P_SDO_CR5);
			writel(0x00000009, g_sdout_base + S5P_SDO_CR6);
			writel(0x0000001c, g_sdout_base + S5P_SDO_CR7);
			writel(0x00000039, g_sdout_base + S5P_SDO_CR8);
			writel(0x0000005a, g_sdout_base + S5P_SDO_CR9);
			writel(0x00000074, g_sdout_base + S5P_SDO_CR10);
			writel(0x0000007e, g_sdout_base + S5P_SDO_CR11);
			break;
		case SDOUT_VTOS_RATIO_7_3:
			writel(0x00000000, g_sdout_base + S5P_SDO_Y0);
			writel(0x00000000, g_sdout_base + S5P_SDO_Y1);
			...
			writel(0x00000000, g_sdout_base + S5P_SDO_Y9);
			writel(0x00000000, g_sdout_base + S5P_SDO_Y10);
			writel(0x00000251, g_sdout_base + S5P_SDO_Y11);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB0);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB1);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB2);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB3);
			writel(0x00000000, g_sdout_base + S5P_SDO_CB4);
			writel(0x00000001, g_sdout_base + S5P_SDO_CB5);
			writel(0x00000006, g_sdout_base + S5P_SDO_CB6);
			writel(0x00000013, g_sdout_base + S5P_SDO_CB7);
			writel(0x00000028, g_sdout_base + S5P_SDO_CB8);
			writel(0x0000003f, g_sdout_base + S5P_SDO_CB9);
			writel(0x00000051, g_sdout_base + S5P_SDO_CB10);
			writel(0x00000056, g_sdout_base + S5P_SDO_CB11);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR1);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR2);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR3);
			writel(0x00000000, g_sdout_base + S5P_SDO_CR4);
			writel(0x00000002, g_sdout_base + S5P_SDO_CR5);
			writel(0x00000005, g_sdout_base + S5P_SDO_CR6);
			writel(0x00000018, g_sdout_base + S5P_SDO_CR7);
			writel(0x00000037, g_sdout_base + S5P_SDO_CR8);
			writel(0x0000005A, g_sdout_base + S5P_SDO_CR9);
			writel(0x00000076, g_sdout_base + S5P_SDO_CR10);
			writel(0x0000007e, g_sdout_base + S5P_SDO_CR11);
			break;
		default:
			pr_err("  invalid composite_ratio parameter(%d)\n",
	...
	SDPRINTK("%d, %d, %d\n", composite_level, composite_ratio, out_mode);

	return SDOUT_NO_ERROR;
}

static enum s5p_tv_sd_err s5p_sdout_init_oversampling_filter_coeff_default(
	enum s5p_tv_o_mode out_mode)
{
	switch (out_mode) {
	case TVOUT_OUTPUT_COMPOSITE:
	...
	case TVOUT_OUTPUT_COMPONENT_YPBPR_INERLACED:
		/*
		temp_reg = (u32)(g_sdout_base + S5P_SDO_OSFC00_0);

		for (i = 0; i < 3; i++) {

			temp_reg = (u32)((i == 0) ?
				g_sdout_base + S5P_SDO_OSFC00_0 :
				(i == 1) ? g_sdout_base + S5P_SDO_OSFC00_1 :
				       g_sdout_base + S5P_SDO_OSFC00_2);
...
*/
enum s5p_tv_sd_err s5p_sdout_init_display_mode(
		...
		enum s5p_sd_order order)
{
	...
	case TVOUT_NTSC_443:
		...
		break;
	default:
		pr_err("invalid disp_mode parameter(%d)\n", disp_mode);
		return S5P_TV_SD_ERR_INVALID_PARAM;
		break;
	}
	...
	case TVOUT_OUTPUT_SVIDEO:
		...
		case S5P_TV_SD_O_ORDER_COMPOSITE_C_Y_CVBS:
			...
			break;
		default:
			pr_err(" invalid order parameter(%d)\n", order);
			return S5P_TV_SD_ERR_INVALID_PARAM;
			break;
		}
		break;
	...
	writel(temp_reg, g_sdout_base + S5P_SDO_CONFIG);

	SDPRINTK(" %d, %d, %d\n", disp_mode, out_mode, order);
	SDPRINTK("0x%08x\n", readl(g_sdout_base + S5P_SDO_CONFIG));
...
{
	writel(SDO_TVOUT_CLOCK_ON, g_sdout_base + S5P_SDO_CLKCON);

	SDPRINTK("0x%x\n", readl(g_sdout_base + S5P_SDO_CLKCON));
}
...
void s5p_sdout_stop(void)
{
	...
	writel(SDO_TVOUT_CLOCK_OFF, g_sdout_base + S5P_SDO_CLKCON);
...
{
	if (active)
		writel(readl(g_sdout_base + S5P_SDO_CLKCON) | SDO_TVOUT_SW_RESET,
			g_sdout_base + S5P_SDO_CLKCON);
	else
		writel(readl(g_sdout_base + S5P_SDO_CLKCON) &
			~SDO_TVOUT_SW_RESET,
			g_sdout_base + S5P_SDO_CLKCON);
...
{
	if (vsync_intr_en)
		writel(readl(g_sdout_base + S5P_SDO_IRQMASK) &
			~SDO_VSYNC_IRQ_DISABLE,
			g_sdout_base + S5P_SDO_IRQMASK);
	else
		writel(readl(g_sdout_base + S5P_SDO_IRQMASK) |
			SDO_VSYNC_IRQ_DISABLE,
			g_sdout_base + S5P_SDO_IRQMASK);
	...
	SDPRINTK("%d)\n", vsync_intr_en);
	SDPRINTK("0x%x)\n", readl(g_sdout_base + S5P_SDO_IRQMASK));
}

void s5p_sdout_clear_interrupt_pending(void)
{
	SDPRINTK("0x%x\n", readl(g_sdout_base + S5P_SDO_IRQ));

	writel(readl(g_sdout_base + S5P_SDO_IRQ) | SDO_VSYNC_IRQ_PEND,
		g_sdout_base + S5P_SDO_IRQ);
...
bool s5p_sdout_get_interrupt_pending(void)
{
	...
	return (readl(g_sdout_base + S5P_SDO_IRQ) | SDO_VSYNC_IRQ_PEND) ? 1 : 0;
}
...
int __init s5p_sdout_probe(struct platform_device *pdev, u32 res_num)
{
	...
	g_sdout_mem = request_mem_region(res->start, size, pdev->name);
	if (g_sdout_mem == NULL) {
		dev_err(&pdev->dev,
	...
	g_sdout_base = ioremap(res->start, size);
	if (g_sdout_base == NULL) {
		dev_err(&pdev->dev,
...
{
	if (g_sdout_base) {
		iounmap(g_sdout_base);
		g_sdout_base = NULL;
	}

	/* remove memory region */
	if (g_sdout_mem) {
		if (release_resource(g_sdout_mem))
			dev_err(&pdev->dev,
