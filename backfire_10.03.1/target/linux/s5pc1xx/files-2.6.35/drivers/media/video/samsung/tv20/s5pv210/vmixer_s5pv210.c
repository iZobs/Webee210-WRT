...
#define VMPRINTK(fmt, args...)	\
	pr_debug("\t\t[VM] %s: " fmt, __func__ , ## args)
#else
#define VMPRINTK(fmt, args...)
#endif
	...
	enum s5p_tv_vmx_layer layer, bool show)
{
	...
	VMPRINTK("layer(%d), S5P_MXR_CFG(0x%x\n)",
	...
	u32 base_addr)
{
	...
	VMPRINTK("layer(%d), base_addr(0x%x), \n", layer, base_addr);
	...
	u32 dst_offs_x, u32 dst_offs_y)
{
	...
	case VM_GPR0_LAYER:
		...
		VMPRINTK("S5P_MXR_GRAPHIC0_DXY(0x%x)\n",
	...
	case VM_GPR1_LAYER:
		...
		VMPRINTK("S5P_MXR_GRAPHIC1_DXY(0x%x)\n",
					...
					u32 src_offs_y)
{
	...
	VMPRINTK("layer(%d), span(%d), width(%d), height(%d), \
		 src_offs_x(%d), src_offs_y(%d))\n",
...
enum s5p_tv_vmx_err s5p_vm_set_bg_color(enum s5p_tv_vmx_bg_color_num colornum,
				...
				u32 color_cr)
{
	u32 reg_value = S5P_MXR_BG_COLOR_Y(color_y) |
...
{
	u32 temp_reg =
		S5P_MXR_MIXER_RESERVED | S5P_MXR_CMU_CANNOT_STOP_CLOCK;
	...
	VMPRINTK("burst(%d), endian(%d) S5P_MXR_STATUS(0x%x)\n",
	...
	enum s5p_tv_o_mode output_mode)
{
	...
	VMPRINTK("mode(%d), output_mode(%d), S5P_MXR_CFG(0x%x)\n",
				...
				u32 dst_height)
{
	...
	case VM_GPR0_LAYER:
		...
		VMPRINTK("S5P_MXR_GRAPHIC0_CFG(0x%x), S5P_MXR_GRAPHIC0_BLANK(0x%x)\n",
			readl(g_mixer_base + S5P_MXR_GRAPHIC0_CFG),
				  ...
				  bool y_dither_enable)
{
	...
	VMPRINTK("cr_dither_enable(%d), cb_dither_enable(%d), y_dither_enable(%d) \
		S5P_MXR_BG_CFG(0x%x)\n",
					    ...
					    u32 coeff2)
{
	...
	VMPRINTK("component(%d), mode(%d), coeff0(%d), coeff1(%d), coeff2(%d)\n",
		component, mode, coeff0, coeff1, coeff2);
...
void s5p_vm_init_csc_coef_default(enum s5p_tv_vmx_csc_type csc_type)
{
	...
	VMPRINTK("S5P_MXR_CM_COEFF_Y(0x%x), \
		S5P_MXR_CM_COEFF_CB(0x%x), \
		S5P_MXR_CM_COEFF_CR(0x%x)\n",
		readl(g_mixer_base + S5P_MXR_CM_COEFF_Y),
		readl(g_mixer_base + S5P_MXR_CM_COEFF_CB),
		readl(g_mixer_base + S5P_MXR_CM_COEFF_CR));
}
	...
	enum s5p_tv_vmx_layer layer, bool en)
{
	...
	VMPRINTK("layer(%d), en(%d), S5P_MXR_INT_EN(0x%x)\n",
		layer, en,
		readl(g_mixer_base + S5P_MXR_INT_EN));
